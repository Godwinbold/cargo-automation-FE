...
const AnalyticalCharts = ({ monthlyData, averageWeight, isLoading }) => {
  ...
  const weightComparisonData =
    averageWeight &&
    averageWeight.previousValue != null &&
    averageWeight.currentValue != null
      ? [
          {
            name: "Previous",
            value: parseFloat(averageWeight.previousValue.toFixed(2)),
            color: "#E0E0E0",
          },
          {
            name: "Current",
            value: parseFloat(averageWeight.currentValue.toFixed(2)),
            color: "#3DA5E0",
          },
        ]
      : [];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#D3EBF8]">
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <h3 className="text-[16px] md:text-[18px] font-bold text-[#1A1A1A]">
              Monthly Performance
            </h3>
            <p className="text-xs md:text-sm text-[#6B6B6B] mt-1">
              Comparison of shipment volume and revenue trends
            </p>
          </div>
      ...
      <div className="bg-white p-6 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] border border-[#D3EBF8]">
        ...
        {averageWeight && averageWeight.growthPercentage !== undefined && (
          <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between">
            ...
            <div
              className={`text-sm font-bold flex items-center gap-1 ${averageWeight.growthPercentage >= 0 ? "text-[#28A745]" : "text-red-500"}`}
            >
              {averageWeight.growthPercentage >= 0 ? "+" : ""}
              {averageWeight.growthPercentage.toFixed(2)}%
            </div>
          </div>
        )}
      </div>
