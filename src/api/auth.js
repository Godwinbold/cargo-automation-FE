...
export const authApi = {
  registerAirlineUser: async (airlineId, userData) => {
    const response = await axiosInstance.post(
      `/auth/airlines/${airlineId}/register-user`,
      userData,
    );
  ...
  acceptInvite: async (payload) => {
    // Assuming the backend has a /auth/accept-invite or use a fallback endpoint if told otherwise.
    // The user mentioned "Here is what I'm sending to the backend"
    const response = await axiosInstance.post('/auth/accept-invite', payload);
    return response.data;
  },
  ...
  // Step 1 & Step 3: Unified endpoint for forgot password
